    ...
    for file_name in os.listdir(gene_sequences_dir):
        if file_name.endswith('.fasta'):
            ...
            if not os.path.exists(output_path) or overwrite:
                # Run MAFFT alignment
                subprocess.run(f'mafft --auto {input_path} > {output_path}', shell=True)
...
def perform_astral_analysis(raxml_trees_dir, output_dir, overwrite):
    ...
    # Define the ASTRAL JAR file path
    astral_jar = "ASTRAL-master/Astral/Astral/astral.5.7.8.jar"
    
    # Check if ASTRAL JAR is present, download and extract if necessary
    if not os.path.exists(astral_jar):
        print("ASTRAL JAR not found. Downloading...")
        try:
            # Download the master zip from GitHub
            astral_url = "https://github.com/smirarab/ASTRAL/archive/refs/heads/master.zip"
            master_zip = "astral_master.zip"
            urllib.request.urlretrieve(astral_url, master_zip)
            print(f"Downloaded {master_zip} successfully.")
            
            # Unzip the downloaded file
            with zipfile.ZipFile(master_zip, 'r') as zip_ref:
                zip_ref.extractall("ASTRAL-master")
            print("Extracted master zip file.")
            
            # Unzip the inner zip file
            inner_zip = "ASTRAL-master/ASTRAL-master/Astral.5.7.8.zip"
            if not os.path.exists(inner_zip):
                print("Error: Inner zip file not found.")
                return

            # Unzip the inner zip file
            with zipfile.ZipFile(inner_zip, 'r') as zip_ref:
                zip_ref.extractall("ASTRAL-master/Astral")
            print("Extracted inner ASTRAL zip file.")

        except Exception as e:
            print(f"Failed to download or extract ASTRAL: {e}")
            return
    ...
    if not os.path.exists(f'{astral_output_prefix}.tre') or overwrite:
        subprocess.run(f'java -jar {astral_jar} -i {combined_tree_file} -b {bootstrap_file_list} -o {astral_output_prefix}.tre -t 2', shell=True)
...
def main():
    ...
    parser.add_argument('-s', '--feature_section_summary', action='store_true', help='Generate section-wise feature summary')
    ...
    parser.add_argument('-o', '--output_dir', required=True, help='Base path for the output files (without extension)')
